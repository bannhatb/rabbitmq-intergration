...
export class DoTestComponent implements OnInit {
  ...
  userData: { userId: number, examId: number, resultUserChooses: { questionId: number, choose: number }[] } = {
    userId: 0,
    examId: 0,
    resultUserChooses: []
  };
  ...
  getExamDetail() {
    this.consumerService.getExamDetailById(this.examId)
      .subscribe({
          next: (res:any) => {
  ...
  submitData() {
    ...
    this.producerService.sendTestUserChoose(dataSubmit)
      .subscribe({
            next: (res:any) => {
              console.log(res)
  ...
  getUserId() {
    this.producerService.getUserIdCurrent()
      .subscribe({
            next: (res:any) => {
  ...
  selectedAnswer(questionId:number, answerId: number) {
    const item = this.resultUserChooses.filter(x => x.questionId === questionId)
